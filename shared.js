    ...
    const TA = {
        ...
        kdj: (c, p = 9) => {
            ...
            for (let i = p - 1; i < c.length; i++) {
                const periodData = c.slice(i - p + 1, i + 1);
                const high = safeMax(periodData);
                const low = safeMin(periodData);
                const rsv = high === low ? 50 : (c[i] - low) / (high - low) * 100;
                rsvArr.push(rsv);
            }
            if (rsvArr.length < 3) return null;
            // 递归计算K、D
            let k = 50, d = 50;
            for (let i = 0; i < rsvArr.length; i++) {
                k = (2 / 3) * k + (1 / 3) * rsvArr[i];
                d = (2 / 3) * d + (1 / 3) * k;
            }
            const j = 3 * k - 2 * d;
            return { k, d, j };
        ...
        atr: (c, p = 14) => {
            if (c.length < p + 1) return null;
            // 用close估算TR（没有high/low时）
            let trSum = 0;
            for (let i = 1; i <= p; i++) {
                const tr = Math.abs(c[i] - c[i - 1]);
                trSum += tr;
            }
            let atr = trSum / p;
            for (let i = p + 1; i < c.length; i++) {
        ...
        adx: (c, p = 14) => {
            ...
            let pdi = pdm14 / (tr14 || 1) * 100, mdi = mdm14 / (tr14 || 1) * 100;
            ...
            for (let i = p + 1; i < c.length; i++) {
                const diff = c[i] - c[i - 1];
                const tr = Math.abs(diff);
                const pdm = diff > 0 ? diff : 0;
                const mdm = diff < 0 ? -diff : 0;
                
                tr14 = (tr14 * (p - 1) + tr) / p;
                pdm14 = (pdm14 * (p - 1) + pdm) / p;
                mdm14 = (mdm14 * (p - 1) + mdm) / p;
                
                pdi = pdm14 / (tr14 || 1) * 100;
                mdi = mdm14 / (tr14 || 1) * 100;
